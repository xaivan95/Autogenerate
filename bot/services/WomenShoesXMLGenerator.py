...
class WomenShoesXMLGenerator(BaseXMLGenerator):
    """Генератор XML для женской обуви"""
    ...
    def generate_ad(self, product: dict, city: str, ad_number: int = 1, metro_station: str = None,
        ...
        print(f"👠 Уровни категории для женской обуви: '{first_level}' - '{second_level}' - '{third_level}'")

        # Категория
        ET.SubElement(ad, "Category").text = "Одежда, обувь, аксессуары"

        # GoodsType
        ET.SubElement(ad, "GoodsType").text = "Женская обувь"

        # Apparel (второй уровень)
